...
impl Term32 {
    ...
    pub const fn term(self) -> Term {
        if self.is_var() {
            Term::Var(self.var_index())
        } else {
            unsafe { Term::Const(self.const_id_unchecked()) }
        }
    }
}
...
impl KB {
    ...
    pub fn get_query_encoding(&self, a: &RawAtom) -> Result<AtomId, KBError> {
        ...
        for t in &a.args {
            match t {
                RawTerm::Variable(v) if v.as_ref() == "_" => {
